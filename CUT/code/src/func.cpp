...
{
	int id=100;
	if(empsal_list.empty())//checking whether the employee list is empty or not 
	{
		id=100;
	}
	else
	...
	while(1)
	{
		...
		try
		{
			...
			{
				if(!(isalpha(emp_name[i]) || emp_name[i]==' '))//checking the name only consists of alphabets
				{
					throw emp_name;
				}
			}
...
void SALARY::view_employee()
{
    ...
    {
        for(SALARY it:empsal_list)
        cout<<endl<<setw(25)<<left<<it.emp_id<<setw(25)<<left<<it.emp_name<<setw(20)<<left<<it.dept<<setw(20)<<left<<it.contact<<setw(20)<<left<<it.project_counter<<endl;
    }
