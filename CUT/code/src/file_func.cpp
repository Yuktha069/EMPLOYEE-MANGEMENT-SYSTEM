...
void PROJECT::project_file_to_list()
{
	...
	if(project_file)
	{
		...
		{
			strcpy( obj.pr_name, pr_obj.pr_name );
			obj.team_mem = pr_obj.team_mem;
			obj.pr_id = pr_obj.pr_id;
...
void PROJECT :: list_to_project_file()
{
	...
	if(!pr_list.empty())
	{
		...
		{
			strcpy( pr_obj.pr_name, it->pr_name );
			pr_obj.team_mem = it->team_mem;
			pr_obj.pr_id = it->pr_id;
			project_file.write((char *) &pr_obj , sizeof(pr_obj));
		}
		project_file.close();
	}
}
