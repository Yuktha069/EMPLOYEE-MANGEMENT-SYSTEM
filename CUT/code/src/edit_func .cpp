...
extern struct emp_details empdetail;
extern struct temp tmp;

extern list <SALARY> empsal_list;// Salary list Declaration
extern SALARY obj;
extern list <PROJECT> pr_list;
extern PROJECT pr_obj;
/******************************************************************************************
 *
...
void SALARY::editdata ()
{
  ...
  if (empsal_list.empty ())
      ...
      while (CHOICE)
	{
      ...
      char n_name[NAME];
      char n_dept[DEPT];
      char n_contact[CONTNO];
      int n_pr[PRO];
      string pr[PRO];
      for (std::list < SALARY >::iterator it = empsal_list.begin ();
	   it != empsal_list.end (); ++it)
	{
	  fg = 0;
	  if (it->emp_id == ed)
	    {
		...
		{
		  cout << "Which detail of Employee Id " << ed <<
		    " you want to change?" << endl;
		  cout << "1. Name" << endl << "2. Department" << endl <<
		    "3. Contact Number" << endl << "4. Project 1" << endl <<
		    "5. Project 2" << endl << "6. Project 3" << endl <<
		    "7.Salary" << endl << "8.Exit" << endl;
		  while (CHOICE)
		    {
		  ...
		  switch (choice)
			...
			while (CHOICE)
			  {
			    cout << "Enter the new name for Employee ID " <<
			      ed << ": " << endl;
			    cin.getline(n_name,NAME);
			    try
			    {
			      for (int i = 0; i < strlen (n_name); i++)
				{
				  if (!(isalpha (n_name[i]) || n_name[i] == ' '))
				    {
				      throw n_name;
				    }
				}
			...
			while (CHOICE)
			  {
			    cout <<
			      "Enter the new Department for Employee ID " <<
			      ed << ": " << endl;
			    cin >> n_dept;
			    try
			    {
		    ...
		    case CONTACT:
		      {
			cout << "Enter the new contact for Employee ID " << ed
			  << ": " << endl;
			cin.ignore (1, '\n');
			cin.getline (n_contact, CONTNO);
			try
			{
			...
			cout << "Enter the Project 1 ID for Employee ID " <<
			  ed << ": " << endl;
			cin >> pr[0];
			try
			{
			...
			n_pr[0] = stoi (pr[0]);
			int flag = FLAG1;
			for (std::list < PROJECT >::iterator st =
			     pr_list.begin (); st != pr_list.end (); ++st)
			  {
			    if (st->pr_id == n_pr[0])
			      {
				flag = FLAG2;
		    ...
		    case PROID2:
		      {
			cout << "Enter the Project 2 ID for Employee ID " <<
			  ed << ": " << endl;
			cin >> pr[2];
			try
			{
		    ...
		    case PROID3:
		      {
			cout << "Enter the Project 3 ID for Employee ID " <<
			  ed << ": " << endl;
			cin >> pr[4];
			try
			{
			...
			while (CHOICE)
			  {
			    cout << "Enter the new salary for Employee ID " <<
			      ed << ": " << endl;
			    cin >> sal;
			    try
			    {
